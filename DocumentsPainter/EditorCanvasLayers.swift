...
/// Текст і штрихи в порядку арт-шарів (ззаду наперед), потім поточний штрих і курсор.
struct EditorCompositeCanvasLayer: View {
    let center: CGPoint
    let scale: CGFloat
    let offset: CGSize
    ...
    var body: some View {
        Canvas { context, size in
            ...
            let orderedVisibleLayerIds = artLayers
                .map(\.id)
                .filter { !hiddenArtLayerIds.contains($0) }
            let visibleLayerIdSet = Set(orderedVisibleLayerIds)
            let hasSearch = !searchQuery.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty

            var visibleLinesByLayer: [UUID: [ImportedTextLine]] = [:]
            visibleLinesByLayer.reserveCapacity(orderedVisibleLayerIds.count)
            for line in importedTextLines {
                guard !hiddenTextLineIds.contains(line.id),
                      visibleLayerIdSet.contains(line.layerId),
                      EditorCanvasHelpers.isTextLineInVisibleContent(line, visible: visibleContent) else { continue }
            ...
            for stroke in strokes {
                guard !hiddenStrokeIds.contains(stroke.id),
                      visibleLayerIdSet.contains(stroke.layerId),
                      stroke.points.count > 1,
                      let bb = EditorCanvasHelpers.boundingRect(of: stroke.points),
                      bb.intersects(visibleContent) else { continue }
                visibleStrokesByLayer[stroke.layerId, default: []].append(stroke)
            }
            ...
            for layerId in orderedVisibleLayerIds {
                ...
                for stroke in visibleStrokesByLayer[layerId] ?? [] {
                    var path = Path()
            ...
            if let bounds = selectedItemBounds, bounds.intersects(visibleContent) {
